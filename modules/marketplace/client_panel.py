...
def show_owner_panel(client_email):
    """Panel para propietarios con fincas"""
    ...
    cursor = conn.cursor()
    cursor.execute("""
        SELECT id, title, surface_m2, price, status, created_at, photo_paths, 
    ...
    for prop in properties:
        prop_id, title, surface_m2, price, status, created_at, photo_paths, owner_name, owner_phone, owner_address = prop
        ...
        with st.expander(f"{status_emoji} {title} - {surface_m2} m²", expanded=True):
            ...
            with col2:
                ...
                st.markdown(f"**📍 Dirección:** {owner_address}")
